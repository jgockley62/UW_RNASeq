...
names(DE) = c('Control-FamilialAD', 'Control-SporadicAD', 'FamilialAD-SporadicAD' )
DE = DE %>% 
...
contrast = makeContrasts( 
  contrasts=c(#"(Controls.male+Familial.male+Sporadic.male)/3-(Controls.female+Familial.female+Sporadic.female)/3", 
              "Familial.male-Controls.male",
              "Familial.male-Sporadic.male",
              "Familial.female-Controls.female",
              "Familial.female-Sporadic.female"),
                         levels = colnames(FIT$coefficients))
FIT.CONTR = contrasts.fit(FIT, contrasts=contrast)
FIT.CONTR = eBayes(FIT.CONTR)
# Get differnetial expression
...
names(DE) = c('Familial-Controls.IN.MALE','Familial-Sporadic.IN.MALE', 'Familial-Controls.IN.FEMALE','Familial-Sporadic.IN.FEMALE')
DE = DE %>% 
  rbindlist(idcol = 'Comparison') %>%
  left_join(biomart_results %>%
              dplyr::select(ensembl_gene_id, hgnc_symbol, percentage_gene_gc_content, gene_length) %>%
...
### Differential expression analysis (with Within FamCohort Between Mut No Mut)
Differential expression is performed on the Disease Cohort x Sex variable by controlling for covariates identified above
...
# Get design matrix
DESIGN = getDesignMatrix(METADATA_Use[, c('Cohort.Mut', setdiff(postAdjustCovars, 'Sex')), drop = F], Intercept = F)
DESIGN$design = DESIGN$design[,linColumnFinder(DESIGN$design)$indepCols]
...
contrast = makeContrasts( 
  contrasts=c(
              "Familial.0-Familial.1",
              "Sporadic.0-Sporadic.1",
              "Sporadic.0-Familial.1",
              "Familial.0-Sporadic.1",
              "Sporadic.0-Familial.0",
              "Sporadic.1-Familial.1"),
                         levels = colnames(FIT$coefficients))
FIT.CONTR = contrasts.fit(FIT, contrasts=contrast)
FIT.CONTR = eBayes(FIT.CONTR)
# Get differnetial expression
...
names(DE) = c('Familial_NoMut-Familial_Mut',
              'Sporadic_NoMut-Sporadic_Mut', 
              'Sporadic_NoMut-Familial_Mut',
              'Familial_NoMut-Sporadic_Mut',
              'Sporadic_NoMut-Familial_NoMut',
              'Sporadic_Mut-Familial_Mut')
DE = DE %>% 
  rbindlist(idcol = 'Comparison') %>%
  left_join(biomart_results %>%
              dplyr::select(ensembl_gene_id, hgnc_symbol, percentage_gene_gc_content, gene_length) %>%
...
#HC
METADATA_Use[ METADATA_Use$disease_cohort == 0, ]$FADNoMut_HC_vs_SporaticNoPSEN1 <- 0
#FAD Controls
METADATA_Use[ METADATA_Use$disease_cohort == 1 & METADATA_Use$ind_mut_status == 0, ]$FADNoMut_HC_vs_SporaticNoPSEN1 <- 0
#Cases
METADATA_Use[ METADATA_Use$disease_cohort == 1 & METADATA_Use$ind_mutation %in% c('2','3','4'), ]$FADNoMut_HC_vs_SporaticNoPSEN1 <- 1
...
### Differential expression analysis (familial mutation - non-mutation excluding PS2 and APP mutations )
Genes that are differentially expressed at an FDR <= 0.05 and folde change of 1.2 are
...
#FAD Controls
METADATA_Use[ METADATA_Use$disease_cohort == 1 & METADATA_Use$ind_mut_status == 0, ]$FAD_NoMut_FAD_PSEN1 <- 0
#Cases
METADATA_Use[ METADATA_Use$disease_cohort == 1 & METADATA_Use$ind_mutation %in% '1', ]$FAD_NoMut_FAD_PSEN1 <- 1
